...
class SummaryRemoteDioImpl implements SummaryRemote {
  ...
  Future<List<RemoteSummaryDto>> getAll(String base) async {
    ...
    Response res1 = await _dioService.dio.get('/latest', queryParameters: {"base" : base});
    final Map res1Map = res1.data["rates"] as Map;

    Response res2 = await _dioService.dio.get('/historical', queryParameters: {
      "base" : base,
      "date" : DateFormat('yyyy-MM-dd').format(DateTime.now()),
    },);
    (res2.data["rates"] as Map).forEach((key, value) {
      result.add(RemoteSummaryDto(name: key as String, curValue: double.parse(res1Map[key]!.toString()), prevValue: double.parse(value.toString())));
    });
