...
class AuthorizationBloc extends Bloc<AuthorizationEvent, AuthorizationState> {
  ...
  AuthorizationBloc(
    this._repository,
          ...
          _repository.currentUser != null
              ? AuthorizationState.authorized(
                  userData: _repository.currentUser!,
                )
    ...
    _sub = _repository.user
        .listen((curr) => add(AuthorizationEvent.authChanged(user: curr)));
  }
  ...
  void _onLoginEmailEvent(
    _LoginEmailEvent event,
    Emitter<AuthorizationState> emit,
  ) async {
  ...
  void _onLoginGoogleEvent(
    _LoginGoogleEvent event,
    Emitter<AuthorizationState> emit,
  ) async {
    final state = this.state;
    if (state is _AuthorizedState) {
    ...
    if (user != null) {
      ...
      emit(
        const AuthorizationState.error(
          error: "Google authorization invalid",
        ),
      );
    }
  }
